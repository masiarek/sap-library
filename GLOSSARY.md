...
| `BSEG-WRBTR` / `BSEG-DMBTR` / `BSEG-DMBE2` / `DMBE3` | Line amount in document currency, in the company code currency, and in the second and third parallel currency. The keys for the last two are in the company code configuration, not on the line. | [Local currency and the parallel currencies](03_Currency/local_and_parallel_currencies/README.md) |
| `BSEG-PSWSL` / `BSEG-PSWBT` | The *update currency* and its amount: the currency this line updated the account's balance in. Local currency when the account has `SKB1-XSALH`, otherwise the document currency — except in a clearing, where it follows the item cleared. What `GLT0-TSL`, `FAGLFLEXT-TSL` and `ACDOCA-TSL` are kept in. | [Update currency](03_Currency/update_currency_pswsl/README.md) |
| `SKB1-XSALH` | *Only balances in local currency* on the company code segment of a G/L account. Decides the update currency, and so whether clearing a foreign-currency item at a new rate produces an exchange rate difference. | [Only balances in local currency](03_Currency/only_balances_in_local_currency/README.md) |
| `T001-XSLTA` | *No forex rate diff. when clearing in LC* on the company code: a clearing entered in local currency takes foreign items at their original local amount. | [Exchange rate differences on clearing](03_Currency/exchange_rate_differences_on_clearing/README.md) |
| `TCURC` / `TCURX` / `TCURT` | Currency keys with their ISO codes; decimal places for currencies that do not have two; texts. Amounts are stored with two decimals and shifted by `TCURX`, so 1,000 JPY is stored as 10.00. | [Currency keys, codes and decimal places](03_Currency/currency_keys_and_decimals/README.md) |
| `TCURR` / `TCURF` / `TCURV` / `TCURN` | Exchange rates by rate type, pair and valid-from date; ratios; rate types with reference currency and inversion; standard quotation per pair. | [Exchange rates](03_Currency/exchange_rates/README.md) |
| `ACDOCA-WSL` / `TSL` / `HSL` / `KSL` / `OSL` … `GSL` | The Universal Journal's amount fields: transaction currency, balance transaction currency (the update currency), company code currency, global currency, and up to eight freely defined currency types per ledger and company code. | [Currencies in the Universal Journal](03_Currency/universal_journal_currencies/README.md) |
...
| `OB22` / `FINSC_LEDGER` | Parallel currencies of a company code in ECC; the ledger and currency-type configuration of S/4HANA that replaced it. | [Ledgers and their currencies](03_Currency/ledgers_and_currencies/README.md) |
| `OB08` / `OB07` / `OBBS` / `ONOT` | Exchange rates; rate types; ratios; standard quotation. | [Exchange rates](03_Currency/exchange_rates/README.md) |
| `OY03` / `OY04` | Currency keys; decimal places. | [Currency keys, codes and decimal places](03_Currency/currency_keys_and_decimals/README.md) |
| `OBA1` | Accounts for exchange rate differences: KDF for open items, KDB for G/L balances, realized and unrealized. | [Exchange rate differences on clearing](03_Currency/exchange_rate_differences_on_clearing/README.md) |
| `OB59` / `FAGL_FC_VAL` / `FAGL_FCV` / `FAGL_FC_TRANS` | Valuation methods; the valuation programs; translation of balances. | [Foreign currency valuation](03_Currency/foreign_currency_valuation/README.md) |
| `OMX2` / `OMX3` | Material Ledger type with its currency types; its assignment to valuation areas. | [Material Ledger currencies](03_Currency/material_ledger_currencies/README.md) |
...
| Term | Meaning | Worked through in |
| :-- | :-- | :-- |
| **Currency type** | The *role* an amount plays — 00 document, 10 company code, 30 group, 40 hard, 50 index-based, 60 global company, 70 object currency, or a customer-defined Y*/Z* type — as opposed to the currency *key*, which says which money. | [Currency types](03_Currency/currency_types/README.md) |
| **Update currency** | The currency a line item updates its account's balance in; see `BSEG-PSWSL`. | [Update currency](03_Currency/update_currency_pswsl/README.md) |
| **Realized / unrealized exchange rate difference** | Realized: posted at clearing, when the local amounts of item and payment differ. Unrealized: posted at period end by valuation on items still open, and reversed or carried as a delta. | [Foreign currency valuation](03_Currency/foreign_currency_valuation/README.md) |
