...
5. A missing rate is a business error: no document, and a message the sender can act on.
...
| # | Type | Document date | Posting date | Amount | Rate | Amount × rate | Expected CAD | What it proves |
| :-- | :-- | :-- | :-- | --: | :-- | --: | --: | :-- |
| FX-01 | `Y1` | 20.08. | 20.08. | 1,000.00 USD | 1.25000 | 1,250 | **1,250.00** | First half of the pair |
| FX-02 | `Y1` | 17.09. | 17.09. | 1,000.00 USD | 1.28000 | 1,280 | **1,280.00** | Same amount, next month: the CAD amount differs by 30.00 |
| FX-03 | `Y1` | 17.09. | 17.09. | 1,000.00 EUR | 1.60000 | 1,600 | **1,600.00** | Cross rate through the reference currency |
| FX-04 | `Y2` | 17.09. | 17.09. | 250.00 EUR | 1.60000 | 400 | **400.00** | Credit memo: posting keys reversed, same translation |
| FX-05 | `Y2` | 17.09. | 17.09. | 333.33 USD | 1.28000 | 426.6624 | **426.66** | Rounds down |
| FX-06 | `Y1` | 17.09. | 17.09. | 123.46 EUR | 1.60000 | 197.536 | **197.54** | Rounds up |
| FX-07 | `Y1` | 20.08. | 20.08. | 123.46 EUR | 1.50000 | 185.19 | **185.19** | Same EUR amount as FX-06, other month: differs by 12.35 |
| FX-08 | `Y1` | 31.08. | 31.08. | 2,500.00 USD | 1.25000 | 3,125 | **3,125.00** | Last day of the month: still August's rate |
| FX-09 | `Y1` | 01.09. | 01.09. | 2,500.00 USD | 1.28000 | 3,200 | **3,200.00** | First day: already September's rate |
| FX-10 | `Y1` | 25.08. | 05.09. | 1,234.56 USD | 1.28000 | 1,580.2368 | **1,580.24** | Document date in August, posting date in September: the posting date decides |
| FX-11 | `Y1` | 17.09. | 17.09. | 0.01 USD | 1.28000 | 0.0128 | **0.01** | Smallest amount does not become 0.00 |
| FX-12 | `Y1` | 17.09. | 17.09. | 1,000.00 CAD | — | — | **1,000.00** | Control record: no translation at all |
...
- **Additional ledger currencies.** If the ledger carries a group currency, each document also has an amount in it, with its own rate type and its own translation date rule. Add a column per currency once you know the configuration.
...
- The **expected amounts are arithmetic** on the reference rates, computed with decimal arithmetic and commercial rounding, not read from a system. The binding expected result for any real run is the *Converted amount* in view 1 of the report, which comes from the standard conversion function.
