...
| [A report that shows which rate a posting would get](exchange_rate_check_report.md) | `Z_FI_EXCH_RATE_CHECK`: rates on a key date with their age, the rate history with gaps, and posted documents against the table rate — plus the three wrong assumptions its first runs exposed. |
