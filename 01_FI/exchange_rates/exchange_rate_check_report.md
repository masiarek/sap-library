...
**Level: 201 · for FI consultants and ABAP developers** — a read-only ABAP report, `Z_FI_EXCH_RATE_CHECK`, with the three questions it answers and the three mistakes its first runs exposed.
...
**The rate comes from the standard lookup, not from a `SELECT`.** View 1 and view 3 call `BAPI_EXCHANGERATE_GETDETAIL` for the rate and its valid-from date, and `CONVERT_TO_LOCAL_CURRENCY` for the amount. Inversion, reference currency, ratios and rounding are then the system's own, not a re-implementation that agrees with it on easy cases only.
...
CALL FUNCTION 'BAPI_EXCHANGERATE_GETDETAIL'
  EXPORTING
    ...
    date       = iv_date
  IMPORTING
    exch_rate  = ls_bapi        " EXCH_RATE = direct, EXCH_RATE_V = indirect
    return     = ls_return.
```
...
The report was activated and run on a development system the same day it was written. Three of its assumptions did not survive, and each failure is the mechanism page in miniature.
