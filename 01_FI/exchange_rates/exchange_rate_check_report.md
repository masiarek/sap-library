...
**Level: 201 · for FI consultants and ABAP developers** — a read-only ABAP report, `Z_FI_EXCH_RATE_CHECK`, with the three questions it answers and the five mistakes its first runs exposed.
...
The report was activated and run on a development system the same day it was written. Five of its assumptions did not survive, and each failure is the mechanism page in miniature.
...
**5. One wrong rate was reported twice.** The first document that disagreed with the table was a 12.00 USD credit memo posted at 1.49600 before current rates existed; the table later said 1.28000 for its translation date. View 3 flagged it correctly — posted 17.95, expected 15.36 — and then also reported the **group currency** amount as wrong: posted 12.00 USD, expected 14.02. The ledger derives the group amount from the *local* amount (`BKPF-BASW2 = 2`), and the report had chained from the **posted** local amount: 17.95 / 1.28 = 14.02. But a USD document with group currency USD should expect its own amount. *Fix:* chain from the **expected** local amount, the way the posting chains from the local amount it calculated: 15.36 / 1.28 = 12.00, no second finding. The wrong number was still useful once — it is how the run revealed which source the group currency is translated from.
...
- **The switch to `READ_EXCHANGE_RATE` (mistake 4) was activated and run the same afternoon.** With `EUR → USD` 1.25000 and `CAD → USD` 1.28000 (indirect) in force, view 1 showed `USD → CAD` 1.28000 and `EUR → CAD` **1.60000**, converted 1,000.00 to 1,280.00 and 1,600.00, and named both legs with their valid-from dates — rate column and amount column now agree. View 2 showed ratios of 1 : 1 read from `TCURF`. The valid-from date is deliberately *not* taken from the function but read from `TCURR`, to avoid depending on a parameter type recalled from memory.
- **View 3 ran later the same day, on one document** — a credit memo of 12.00 USD in a company code with local currency CAD and group currency USD. Document rate and table rate were both 1.49600, posted and expected local amount both 17.95 (12.00 × 1.496 = 17.952), posted and expected group currency amount both 12.00, and the light was yellow because the table rate was 9,390 days old on the translation date. An independent extract of the same document showed the same three amounts. One matching document proves the join, the header fields and the arithmetic, not the red branches.
- **The red branch fired that evening.** After current rates had been maintained, view 3 listed three documents: an EUR invoice (rate 1.60000, 1,600.00 local, 1,250.00 group — all green); a local currency invoice, listed only for its group currency amount (5,000.00 → 3,906.25, green); and the credit memo posted earlier at the old rate — red, 16.875 % rate deviation, 2.59 difference. That is the late-load case of the [mechanism page](how_a_posting_picks_its_rate.md) §3, produced by accident. The fix for mistake 5 was activated afterwards and re-run on the same three documents: the credit memo stayed red for its local amount (17.95 against 15.36) and now showed 12.00 against 12.00 in the group currency. View 3 needs S/4HANA, because it joins `BSEG`.
